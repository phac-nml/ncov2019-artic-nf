...
A few changes and additions have been made to optimize running this pipeline for the NML and automating the outputs to their respective locations. The basics of the pipeline should still work with no issues other than there being slightly modified qc output files and the need to set a cache directory with the ncov-tools environment before running the pipeline.
...
- Automation of running [ncov-tools](https://github.com/jts/ncov-tools) on the data
    ...
    - If you want negative control check to run you will have to format the negative control name in the `samplesheet.tsv` as seen above
- Singular qc csv file from a variety of sources
- Outputs files to show samples with no / too few reads that are filtered out
- Automated upload to IRIDA
